...
async def AssessmentQuestionSelection(assessmentInput:AssessmentQuestions):
    try:
        ...
        cutt_off=questions_cnt*(1/3)
        query=f"select subject_id from exam_subjects where class_exam_id={exam_id}"
        ...
        if len(test_status) == 0:
            ...
            for i in range(len(subject_ids)):
                query1 = f'select qb.question_id, qb.subject_id,qb.chapter_id, qb.topic_id, qb.question, qb.template_type, qb.difficulty_level, \
                                            ...
                                            from {question_bank_name} qb LEFT JOIN question_bank_passage_inst b ON b.id = qb.passage_inst_id where qb.subject_id={int(subject_ids[i])}\
                                            order by rand() limit {int(cutt_off)}'
