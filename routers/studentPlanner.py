...
from schemas.StudentPlanner import StudentPlanners,UpdateStudentPlanners
router = APIRouter(
    prefix='/api',
...
@router.post('/update-student-planner', description='Update student planner details ')
async def update_student_planner(studentp: UpdateStudentPlanners):
    #print(studentp)
    try:
        conn = Tortoise.get_connection("default")
        student_id=studentp.student_id
        ...
        todate = datetime.strptime(studentp.date_to,"%Y-%m-%d").date()
        #check if planner already exists for current week in db
        checkquery=f'SELECT id FROM student_planner where date_from= "{fromdate}" and date_to="{todate}" and student_id={student_id} and test_completed_yn="N";'
        records=await conn.execute_query_dict(checkquery)
        if records:
            deletequery=f'DELETE FROM student_planner where date_from= "{fromdate}" and date_to="{todate}" and student_id={student_id} and test_completed_yn="N";'
            await conn.execute_query_dict(deletequery)
        #print("Records length: "+str(len(records)))
        exam_id=studentp.exam_id
        chapter_id=json.loads(studentp.chapter_id)
        chapter_id_list = [int(i) for i in chapter_id]
...
async def get_student_planner(student_id:int=0):
    try:
        ...
        conn = Tortoise.get_connection("default")
        query=f'select sp.id,sp.exam_id,sp.subject_id,question_count,test_time_in_min,sp.chapter_id, esc.chapter_name ,date_from,date_to,test_completed_yn from student_planner as sp join exam_subject_chapters as esc on sp.chapter_id=esc.chapter_id where student_id={student_id} '
        result= await conn.execute_query_dict(query)
